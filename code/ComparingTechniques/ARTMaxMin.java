...
//zqj
	public ARTMaxMin(String coverageFile){
...
public int[] getSelectedTestSequence(){
	...
	while(selected.size() < len){
		...
		while(true){
			...
			//if(this.getCoveredNumber(this.CoverageMatrix[newCandiadteIndex]) == 0) continue;
			this.mergeIntoCurrentArray(covered, this.CoverageMatrix[newCandiadteIndex]); //Merge the new statements/methods/branches coverage into the covered array.
...
//Calculate the number of '1' in the array.
	...
	public static void main(String[] args){
		ARTMaxMin art = new ARTMaxMin("statement_matrix.txt");
